...
void Bcor::validateFileIntegrity() const {
  ...
  if (actual_size < 32) {  // Minimum header size
    throw std::runtime_error("File '" + fname +
                             "' is too small to be a valid bcor file");
  }
}
...
void Bcor::readHeader() {
  ...
  if (magic_str == "bcor1.1") {
    ...
    throw std::runtime_error("File '" + fname +
                             "' is not a valid bcor file! Magic: " + magic_str);
  }
...
void Bcor::fillBuffer(uint64_t target_offset) const {
  ...
  size_t to_read =
      std::min(BUFFER_SIZE, static_cast<size_t>(fsize - target_offset));
  ...
  if (fh.gcount() != static_cast<std::streamsize>(to_read) && !fh.eof()) {
    throw std::runtime_error(
        "Failed to read expected amount of data from file");
  }
}
...
double Bcor::readCorrPairTyped(uint32_t snp_x, uint32_t snp_y) const {
  ...
  return std::ldexp(static_cast<double>(val), -1 * (8 * Traits::bytes - 2)) -
         1.0;
}
...
Rcpp::DataFrame Bcor::getMeta() {
  ...
  return Rcpp::DataFrame::create(
      Rcpp::Named("rsid") = rsid, Rcpp::Named("position") = pos_vec,
      Rcpp::Named("chromosome") = chromosome, Rcpp::Named("allele1") = allele1,
      Rcpp::Named("allele2") = allele2, Rcpp::_["stringsAsFactors"] = false);
}
...
arma::mat Bcor::readCorr(const std::vector<int> &snps1,
                         const std::vector<int> &snps2) {
...
arma::sp_mat Bcor::readCorrSparse(const std::vector<int> &snps,
                                  double threshold) {
...
Rcpp::List bcor_open(std::string filename, bool read_header = true) {
  ...
  return Rcpp::List::create(
      Rcpp::Named("ptr") = ptr, Rcpp::Named("filename") = bcor->getFname(),
...
// [[Rcpp::export]]
arma::mat bcor_read_corr(
    SEXP bcor_ptr, Rcpp::Nullable<Rcpp::IntegerVector> snps = R_NilValue) {
  Rcpp::XPtr<Bcor> bcor(bcor_ptr);
...
// [[Rcpp::export]]
arma::mat bcor_read_corr2(SEXP bcor_ptr, Rcpp::IntegerVector snps1,
                          Rcpp::IntegerVector snps2) {
  Rcpp::XPtr<Bcor> bcor(bcor_ptr);
...
// [[Rcpp::export]]
arma::sp_mat bcor_read_corr_sparse(
    SEXP bcor_ptr, Rcpp::Nullable<Rcpp::IntegerVector> snps = R_NilValue,
...
arma::sp_mat Bcor::readCorrSparse(const std::vector<int> &snps1,
                                  const std::vector<int> &snps2,
                                  double threshold) {
